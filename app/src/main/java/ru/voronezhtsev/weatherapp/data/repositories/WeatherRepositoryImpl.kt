...
class WeatherRepositoryImpl(
    ...
    override fun loadFromNetwork(cityId: Long) = weatherService.getWeather(cityId.toString(), APP_ID)
            .flatMap { response ->
                weatherDao.save(convertTemp(response))
    ...
    private fun convertTemp(response: WeatherResponse) = WeatherResponse(response.id,
            Main(response.main.temp - 273.15), response.weather, response.name,
            Date().time)
}
